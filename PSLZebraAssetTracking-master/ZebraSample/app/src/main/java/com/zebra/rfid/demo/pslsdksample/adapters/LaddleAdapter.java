...
public class LaddleAdapter extends BaseAdapter {
    private Context context;
    private LayoutInflater inflater;
    private List<Laddle> lst;

    public LaddleAdapter(Context context, List<Laddle> lst) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        LaddleAdapter.MyViewHolder mViewHolder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.transaction_types_adapter_layout, parent, false);
            mViewHolder = new LaddleAdapter.MyViewHolder(convertView);
            convertView.setTag(mViewHolder);
        } else {
            mViewHolder = (LaddleAdapter.MyViewHolder) convertView.getTag();
        }
        //  SFModal crm = lst.get(position);//change by bapu 5-sep-2018 to desc order
        final Laddle crm = lst.get(position);
        mViewHolder.textName.setText(crm.getDescription());
