...
public class DestinationAdapter extends BaseAdapter {
    ...
    public DestinationAdapter(Context context, List<TasksModal> lst) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        DestinationAdapter.MyViewHolder mViewHolder;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.transaction_types_adapter_layout, parent, false);
            mViewHolder = new DestinationAdapter.MyViewHolder(convertView);
            convertView.setTag(mViewHolder);
        } else {
            mViewHolder = (DestinationAdapter.MyViewHolder) convertView.getTag();
        }
        //  SFModal crm = lst.get(position);//change by bapu 5-sep-2018 to desc order
        final TasksModal crm = lst.get(position);
        mViewHolder.textName.setText(crm.getTask_destination_location());
